...
elgendi <- function(signal, srate = 360L, lowcut_f1 = 8L, highcut_f2 = 21L, filter_order = 3L, qrs_win1 = 35, 
    ...
    slackness_red = FALSE,
    slackness_win1 = 0.200, slackness_win2 = 0.140) {
    ...
    # beat_win2: W2: Integer (samples). For a sampling frequency (SF) of 360 Hz. Default = 220.
        # In Porr 2019, window2 is multiple of window1 so that a window1 will not overlap two window2
        # In contrast, Elgendi 2010 & Elgendi 2012 allow window1 to overlap two window2
    ...
    signal_filt <- signal_filt[(srate + 1):(length(signal_filt) -
        srate)]
    # signal squaring as in Elgendi 2013. Porr 2019 has been using abs()
    signal_squared <- signal_filt^2
    
    # Rolling means
    window1 <-  trunc(qrs_win1 * srate / srate_ref)
        mwa_qrs <- data.table::frollmean(signal_squared, window1, align = "center") 
    # Center alignment as Elgendi 2013 (right alignment in Elgendi 2010 and Porr 2019)
    window2 <-  trunc(beat_win2 * srate / srate_ref)
        mwa_beat <- data.table::frollmean(signal_squared, window2, align = "center")
    
    window3 <-  trunc(offset_win3 * srate)
        mwa_noise <- data.table::frollmean(signal_squared, window3, align = "right")
    ...
    # unfold cleaned, numbered blocks and "silent" zones
    block.clean <- rep(block.le$values, block.le$lengths)
    # peak height on the squared scale
    height <- signal_squared
    height[!is.na(block.clean)] <- unlist(tapply(height, block.clean, function(x) data.table::fifelse(x == max(x), max(x), NA)))
    height[is.na(block.clean)] <- NA
    # peak location
    idx <- seq_along(height)
    loc <- data.table::fifelse(is.na(height) | height == 0L, NA, idx)
...
ad100_qrs <- elgendi(signal = 1000 * ad100[, MLII], srate = 360L, lowcut_f1 = 8L, highcut_f2 = 21L, filter_order = 3L, qrs_win1 = 35L, 
    beat_win2 = 220L, srate_ref = 360L, offset = 0.08, offset_win3 = 10L,
    slackness_red = TRUE,
    slackness_win1 = 0.200, slackness_win2 = 0.140)
...
plot(signal_squared ~ idx, data = ad100_qrs0, type = "l")
lines(mwa_qrs ~ idx, data = ad100_qrs0, type = "l", col = 2) 
lines(mwa_beat ~ idx, data = ad100_qrs0, type = "l", col = 3) 
lines(I(20e+04 * block.clean) ~ idx, data = ad100_qrs0, type = "l", col = 5) 
points(signal_squared ~ loc, data = ad100_qrs0, col = "darkred")
points(signal_squared ~ loc.sr, data = ad100_qrs0, col = "#036830", pch = 3)

library(plotly)
plot_ly(ad100_qrs, x = ~idx, y = ~signal, type = "scatter", linetype = 1, mode = "lines") %>% 
add_markers(x = ~loc[!is.na(loc)], y = ~signal[!is.na(loc)], inherit = FALSE) %>% 
